...
class MHADecoder(torch.nn.Module):
    ...
    def __init__(self,
        ...
        Args:
            ...
            hidden_size: Number of hidden dimensions in the state network.
            mem_size: Number of output dimension from the state network
            clf_size: Number of hidden dimension in the decision network.
            leaky_slope: Slope in the leaky ReLU.
        """
    ...
    def forward(self, embed_x: torch.Tensor, state: torch.Tensor):
        ...
        Args:
            embed_x: (sampling number K, num jobs N, num features F)
            state: (sampling number K, num jobs N, num features F)
        Return:
            Probability of each job, with shape (sampling number K, num jobs N)
        """
        # Make job states
