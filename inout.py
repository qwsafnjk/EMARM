...
def read_basic(f_path: str, sep: str = ' ', device: str = 'cpu'):
    """
    Load the basic information about a FJSSP instance.
    ...
    with open(f_path) as f:
        ...
        # Load the instance
        instance = torch.empty((n, 4 * m), dtype=torch.float32, device=device)
        for j in range(n):
        ...
        # Load the fuzzy makespan of a reference solution, if any
        ms = 1.
        try:
            _ms = next(f)
            if _ms != '':
...
def extract_features(num_j: int, num_m: int, costs_t: torch.Tensor, costs_t1: torch.Tensor, costs_t2: torch.Tensor, 
                     costs_t3: torch.Tensor, machines_t: torch.Tensor, device: str = 'cpu'):
    """
    ...
    _max = costs_t3.max()
    costs = costs_t / _max
    costs1 = costs_t1 / _max
    costs2 = costs_t2 / _max
    costs3 = costs_t3 / _max
    # Job-related
...
def load_data(path, device: str = 'cpu', sep: str = ' '):
    """
    Load a FJSSP instance from path and return a PyTorch Data object.
    ...
    for i in range(1, instance.shape[1] - 2, 4):
        selected_columns = (instance[:, i] + 2 * instance[:, i + 1] + instance[:, i + 2]) / 4
        costs_columns.append(selected_columns)

    costs = torch.stack(costs_columns, dim=1)
    costs1 = instance[:, 1::4]
    costs2 = instance[:, 2::4]
    costs3 = instance[:, 3::4]
    machines = instance[:, ::4].long()

    # Make the disjunctive graph
    edges_t = graph_edges(num_j, num_m, machines, device=device)

    # Prepare the features
    x = extract_features(num_j, num_m, costs, costs1, costs2, costs3, machines, device=device)
